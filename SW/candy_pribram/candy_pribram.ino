  ...
  for(int x=0; x<MEASUREMENTS; x++)  
  {
    ...
    */
    ...
    for(int n=0; n<18200; n++) // cca 13 s
    {
      ...
      sensor += 511; // Add offset to ground (for 0 resistors)
  ...
  
    {
      ...
      //int flux=0;
      //for(int n=zero; n<(1024); n++)
      //{
      //  flux += buffer[n];
      //}
    
      //int loDose = 0;
      //if (zero > 0) loDose=buffer[zero-1];
    
      int flux=0;
      for(int n=noise+13; n<(1024); n++)
      {
        flux += buffer[n];
      }
      
      toMonitor += String(count++) + "," + String(noise) + "," + String(zero) + "," + String(flux) + "*";
      swSerial.println(toMonitor);
      
      dataString += String(noise) + "," + String(zero) + "," + String(flux);
